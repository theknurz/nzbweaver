    ...
    if (pair_find(config_downloads, "path")) {
        ...
        if (stat(pair_find(config_downloads, "path"), &pdir) == 0) {
            if ((pdir.st_mode & S_IFMT) == S_IFDIR) {
                // good path here.
                nzbPath = mw_mprintfv("%s/%.*s/", pair_find(config_downloads, "path"), (int)(nzbname_end-nzbname_start), nzbname_start);
        ...
        nzbPath = mw_mprintfv("%s/%.*s/", userInfo->pw_dir, (int)(nzbname_end-nzbname_start), nzbname_start);
    }
...
void *mw_thread_work (void* arg) {
    ...
    // if either tree is finished or quit download is set to true
    while (nzb_tree_next_segment(&curFile, &curSeg)) {
        // article-number or segment-number w. directories.. 
        char *fullfilePath;
        fullfilePath = mw_mprintfv ("%s/%s", nzb_tree.download_destination, curSeg->articleID);
        int fd_outfile;
        ...
        if (nntp_get_article(serverInfo, curSeg->articleID, &recvBuffer)) {
            ...
            if (nntp_get_yenc_header_begin_end(recvBuffer, &yEncStart, &yEncEnd)) {
                ...
                // create the file, truncate the temp. file
                fd_outfile = open(fullfilePath, O_CREAT | O_TRUNC | O_WRONLY, S_IWUSR | S_IRUSR);
                if (fd_outfile == -1) {
                    LOG_MESSAGE(true, "Could not write to temporary file:%s, errno %d, (%s)\n", fullfilePath, errno, strerror(errno));
                    continue;
                }
                ...
                *userData->downloaded += binarySize;
                ...
                ssize_t writeChk = write(fd_outfile, binary, binarySize);
                if ((size_t)writeChk != binarySize) {
                    LOG_MESSAGE(true, "Could not write to %s (%lu out of %lu written), errno: %d, (%s)\n", fullfilePath, writeChk, binarySize, errno, strerror(errno));
                    curSeg->decoded_bytes = writeChk;
                }
                close(fd_outfile);
                fd_outfile = -1;
                free (binary);
...
void mw_join_parts(struct NZBFile *curFile) {
    ...
    char *release_file_name = mw_mprintfv("%s/%s", nzb_tree.download_destination, curFile->yenc_filename);

    if (!curFile) {
        LOG_MESSAGE(false, "mw_join_parts: *arg == NULL\n");
        return;
    }
    ...
    for (unsigned int i = 0; i < curFile->segmentsSize; i++) {
        ...
        char *segmentFile = mw_mprintfv("%s%s", nzb_tree.download_destination, curFile->segments[i].articleID);
        char *joinbuffer;
...
/// @param 
void mw_post_work(void) {
    unsigned int filecnt;
    extern struct NZB nzb_tree;
    struct NZBFile *smallest_parfile = NULL;

    for (filecnt = 0; filecnt < nzb_tree.max_files; filecnt++) {
        q_printf ("Assembling file %i out of %i\r", filecnt+1, nzb_tree.max_files);
        mw_join_parts(&nzb_tree.files[filecnt]);
    }
    ...
    q_printf("Renaming: smallest_parfile%sfound, checking par2 for file names.", smallest_parfile == NULL ? "not " : " ");
    ...
    if (!smallest_parfile) {
        for (filecnt = 0; filecnt < nzb_tree.max_files; filecnt++) {
            ...
            char *nzb_name = contains_filename(nzb_tree.files[filecnt].filename);
            char *yenc_name = contains_filename(nzb_tree.files[filecnt].yenc_filename);
            char *oldFullName;
            char *newFullName;

            // only use nzb's filename if no yenc_filename was set (should never happen)!
            if (!yenc_name && nzb_name) {
                oldFullName = mw_mprintfv("%s/%s", nzb_tree.download_destination, curFile->yenc_filename);
                newFullName = mw_mprintfv("%s/%s", nzb_tree.download_destination, curFile->filename);
        ...
        char *par2File = mw_mprintfv("%s/%s", nzb_tree.download_destination, smallest_parfile->yenc_filename);
        if (!get_par2_filenames(par2File)) {
        ...
        for (filecnt = 0; filecnt < nzb_tree.max_files; filecnt++) {
            ...
            if ((!string_ends_width(testfile->filename, ".par2")) && (!string_ends_width(testfile->yenc_filename, ".par2"))) {
                ...
                for (unsigned int par_names_cnt = 0; par_names_cnt < par_filenames_length; par_names_cnt++) {
                    // 1 successful check is enough!
                    if (strcmp(par_filenames[par_names_cnt], testfile->filename) == 0) {
                        is_yenc_name = false;
                        break;
                    }
                    if (strcmp(par_filenames[par_names_cnt], testfile->yenc_filename) == 0) {
                        break;
                    }
...
void mw_print_overview(void) {
    ...
    printf ("NZB:%s\tSize:%s\t", nzb_tree.name, mw_val_to_hr_string(nzb_tree.release_size));
