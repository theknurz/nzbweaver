...
/* 
/// @brief converts a yEnc-based message to a binary
/// @param bufferIn the buffer in with the message, ybegin/yend are mandatory in this buffer.
/// @param binaryOut pointer to pointer (NULL) with the output-buffer
/// @param outSize the size of the binary output buffer
/// @param crcOK -1 if no CRC was present, 0 if crc was present but not the same, 1 if crc was present and was the same as in the meta
/// @return if the message was y-De-enc'd successfully
bool nntp_get_binary_from_article (char *bufferIn, char **binaryOut, size_t *outSize, int *crcOK) {
    ...
    size_t binarySize = 0;
    uint32_t crc32 = 0, rapidcrc32;
    bool hasParts = false;

    char *yEncDataStart = NULL, *yEncDataEnd = NULL;;
    ...
    if (pcre2_match(comp_re_start, (const unsigned char*)bufferIn, PCRE2_ZERO_TERMINATED, 0, 0, match_data, NULL) >= 0) {
        PCRE2_SIZE *ovector = pcre2_get_ovector_pointer(match_data);
        char* fnd = strndup(bufferIn+ovector[2], (int)(ovector[3]-ovector[2]));
        struct pair *yBeginMeta = nntp_get_yenc_meta(fnd);
        if (pair_find(yBeginMeta, "part"))
            hasParts = true;
        pair_destroy(&yBeginMeta);
        yEncDataStart = bufferIn + ovector[1];
        free (fnd);
    }
    pcre2_match_data_free(match_data);

    // this is necessary bc if we have a =ypart section, the yenc data starts after this, not after =ybegin
    if (hasParts) {
        match_data = pcre2_match_data_create_from_pattern(comp_re_part, NULL);
        if (pcre2_match(comp_re_part, (const unsigned char*)bufferIn, PCRE2_ZERO_TERMINATED, 0, 0, match_data, NULL) >= 0) {
            PCRE2_SIZE *ovector = pcre2_get_ovector_pointer(match_data);
            yEncDataStart = bufferIn + ovector[1];
        }
    }
    ...
    if (pcre2_match(comp_re_end, (const unsigned char*)bufferIn, PCRE2_ZERO_TERMINATED, 0 , 0, match_data, NULL) >= 0) {
        PCRE2_SIZE *ovector = pcre2_get_ovector_pointer(match_data);
        char *fnd = strndup(bufferIn+ovector[2], (int)(ovector[3]-ovector[2]));
        struct pair *yEndMeta = nntp_get_yenc_meta(fnd);
        if (pair_find(yEndMeta, "crc32")) {
            crc32 = strtoul(pair_find(yEndMeta, "crc32"), NULL, 16);
        } else if (pair_find(yEndMeta, "pcrc32")) {
            crc32 = strtoul(pair_find(yEndMeta, "pcrc32"), NULL, 16);
        } 
        if (pair_find(yEndMeta, "size")) {
            binarySize = atoi(pair_find(yEndMeta, "size"));
        }
        yEncDataEnd = bufferIn+ovector[0];
    }

    *binaryOut = (char*)calloc(binarySize != 0 ? binarySize : strlen(bufferIn), 1);
    *outSize = rapidyenc_decode((const void*)yEncDataStart, *binaryOut, (size_t)(yEncDataEnd-yEncDataStart));
    rapidcrc32 = rapidyenc_crc((void*)*binaryOut, *outSize, 0);
